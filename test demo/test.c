...
int check_random(int chosen[], int current, int ran)// Hàm check xem số được random đã xuất hiện chưa
{
    ...
    {
        if(chosen[i] == ran) return 0;
// Nếu đã xuất hiện thì trả về 0 ngược lại trả về 1
    }
    return 1;
}
...
int main()
{
    ...
    char c1,c2,s[100];
    fpt = fopen("cauhoi.txt","r");// Mở file câu hỏi
...
// in ra các câu hỏi và đáp án
    ...
    int chosen[10];// toạ mảng các số được random
    ...
    FILE *fot = fopen("dethi.txt","w");// mở file đề thi
    if(fot == NULL) printf("\nERROR opening file");
    else
    {
        printf("\n================================");
